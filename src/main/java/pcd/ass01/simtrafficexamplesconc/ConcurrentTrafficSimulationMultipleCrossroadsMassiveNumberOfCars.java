...
/**
 * This class represents a simulation of a traffic system with multiple roads and crossroads with a massive number of cars.
 * The simulation is concurrent and it is based on the {@link pcd.ass01.simtrafficconc} package.
 */
public class ConcurrentTrafficSimulationMultipleCrossroadsMassiveNumberOfCars extends ConcurrentAbstractSimulation {

    private boolean guiEnabled = false;

    public ConcurrentTrafficSimulationMultipleCrossroadsMassiveNumberOfCars(ExecutionFlag threadFlag, boolean guiEnabled) {
        super(threadFlag);
        this.guiEnabled = guiEnabled;
    }

    public ConcurrentTrafficSimulationMultipleCrossroadsMassiveNumberOfCars(ExecutionFlag threadFlag, boolean guiEnabled, int seed) {
        super(threadFlag, seed);
        this.guiEnabled = guiEnabled;
    }

    public ConcurrentTrafficSimulationMultipleCrossroadsMassiveNumberOfCars() {
    ...
    public void setup() {
        ...
        for (int i = 0; i < numCars; i++) {
            ...
            CarAgent car = new CarAgentExtended("car-" + i, env, road, i * 10, acc, dec, gen.nextDouble(3.5, 4.5), monitor);
            this.addAgent(car);
        }
